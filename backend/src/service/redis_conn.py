...
database_logger = setup_logging("Database_logger")
        ...
        if self.redis is None:
            for attempt in range(3):
                try:
                    ...
                    main_logger.debug(f"⚠️ Redis connection failed (attempt {attempt + 1}/3): {e}")
                    await asyncio.sleep(2)

            main_logger.debug("❌ Could not connect to Redis after 3 attempts")
            raise RuntimeError("Redis connection failed")

    async def get_redis(self):
        if self.redis is None:
            main_logger.debug("🔄 Reconnecting to Redis...")
            await self.connect()
